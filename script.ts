...
function renderPanel(auc: AuctionResponse, data: AuctionPageResponse, appender: HTMLDivElement = $$(".auc")) {
...
$$("#pvgo").addEventListener("click", async () => {
    ...
    let res = await fetch("https://sky.shiiyu.moe/api/v2/profile/" + $$("#username").value);
    let data = await res.json();
